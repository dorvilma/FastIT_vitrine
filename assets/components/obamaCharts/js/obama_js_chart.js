...
nytg.formatNumber = function(n,decimals) {
    ...
    if (n >= 1000000000) {    //100.000.000
        suffix = " million"
        n = n / 1000000000      //  1.000.000.000
        decimals = 1
    } else   {   // 10.000.000
        suffix = " thousands"
        n = n / 1000000             //10.000.000
        decimals = 0
    } ;
...
nytg.Chart = function(){
  return {
    ...
    rScale          : d3.scale.pow().exponent(0.5).domain([0,5000000]).range([1,90]),
    radiusScale     : null,
    ...
    init: function() {
      ...
      d3.select("#nytg-scaleKey").append("circle")
        .attr('r', this.radiusScale(1000000)) //100.000.000
        .attr('class',"nytg-scaleKeyCircle")
        .attr('cx', 30)
        .attr('cy', 30);
      d3.select("#nytg-scaleKey").append("circle")
        .attr('r', this.radiusScale(100000))    //10.000.000
        .attr('class',"nytg-scaleKeyCircle")
        .attr('cx', 30)
        .attr('cy', 50);
      d3.select("#nytg-scaleKey").append("circle")
        .attr('r', this.radiusScale(10000))   //1.000.000
        .attr('class',"nytg-scaleKeyCircle")
        .attr('cx', 30)
        .attr('cy', 55);
      ...
      for (var i=0; i < nytg.category_data.length; i++) {
        ...
        if (catLabel === "Other") {
         ...
         nytg.category_data[i].position =  this.categoryPositionLookup[cat]
    ...
    staticDepartment: function(alpha) {
      ...
      return function(d){
        var targetY = 0;
        var targetX = 0;
//nytg.category_data[i]


for(var i = 0 ; i <= nytg.category_data.length;i++){
  if(nytg.category_data[i].category === d.group){
    targetX = nytg.category_data[i].position.x;
    targetY = nytg.category_data[i].position.y;
    break ;
  }
}
